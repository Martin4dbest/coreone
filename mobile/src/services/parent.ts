...
export async function downloadParentStudentResultsPdf(
  studentId: number
) {
  const apiRoot =
  ...
  const url =
    `${apiRoot}/api/v1/parents/me/students/${studentId}/results/pdf`;
  ...
  if (Platform.OS === "web") {
    ...
    if (!response.ok) {
      let detail =
        `Unable to download report PDF (${response.status}).`;
