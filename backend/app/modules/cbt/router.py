...
async def student_available_exams(
    ...
    for exam in exams:
        ...
        # The actual number of questions available to the student
        # must come from the questions attached to this exam.
        # Do not use exam.total_questions here because that value can
        # represent the configured/question-bank target rather than
        # the number of questions actually attached to the exam.
        question_count_result = await db.execute(
            select(CBTQuestion.id).where(
                CBTQuestion.exam_id == exam.id
            )
        )

        actual_question_count = len(
            question_count_result.scalars().all()
        )
